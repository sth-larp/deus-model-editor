...
export class JsonTextLine {
    public isTriggerCollapsed = false;
...
// Constants
    ...
    static genSimpleArrayView(obj: any, indent: number): JsonTextLine[] {
        ...
        for (let i = 0; i < obj.length; i++) {
            const simpleVal = this.genSimpleView(obj[i], indent);
    ...
    static genArrayView(obj: any, indent: number): JsonTextLine[] {
        ...
        const retText: JsonTextLine[] = [ new JsonTextLine( this.toSpan('[', 'leftBracket'),
                                                          'none',
                                                          true,
        ...
        const lineIndent = this.indentString.repeat(indent + 1);

        for (let i = 0; i < obj.length; i++) {
             const nodeLines = this.genModelValueView(obj[i], indent + 1);
    ...
    static genObjectView(obj: any, indent: number): JsonTextLine[] {
        ...
        const retText: JsonTextLine[] = [ new JsonTextLine(
        ...
        for (const p in obj) {
            ...
            const nodeLines: JsonTextLine[] = this.genModelValueView(obj[p], indent + 1);
    ...
    static genSimpleView(obj: any, indent: number): JsonTextLine {
        const ret: JsonTextLine = new JsonTextLine('');
